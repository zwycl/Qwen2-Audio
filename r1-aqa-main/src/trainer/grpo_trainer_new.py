...
# Handle optional imports for compatibility with different TRL versions
try:
    from trl.extras.profiling import profiling_context, profiling_decorator
except ImportError:
    # Fallback: no-op decorators/contexts
...
try:
    from trl.import_utils import is_deepspeed_available, is_vllm_available
except ImportError:
...
class GRPOTrainer(Trainer):
    ...
    @profiling_decorator
    def _get_per_token_logps(self, model, input_ids, attention_mask, features_values, features_masks, logits_to_keep):
        logits = model(input_ids, attention_mask=attention_mask, input_features=features_values, feature_attention_mask=features_masks).logits
    ...
    def _generate_and_score_completions(
        ...
        prompt_inputs = self.processing_class(text=prompts_text, audio=audios, sampling_rate=16000, return_tensors="pt", padding=True)
        ...
        if self.args.use_vllm:
            ...
            with unwrap_model_for_generation(
                ...
                prompt_completion_ids = unwrapped_model.generate(
                    prompt_ids, attention_mask=prompt_mask, input_features=features_values, feature_attention_mask=features_masks, generation_config=self.generation_config
                )
        ...
        # Expand prompt_mask to match num_generations
        ...
        # Expand features to match the number of generations
        ...
        with torch.no_grad():
            ...
            if self.num_iterations > 1:
                old_per_token_logps = self._get_per_token_logps(
                    self.model, prompt_completion_ids, attention_mask, features_values, features_masks, logits_to_keep
                )
            else:
            ...
            if self.beta == 0.0:
                ...
                ref_per_token_logps = self._get_per_token_logps(
                    self.ref_model, prompt_completion_ids, attention_mask, features_values, features_masks, logits_to_keep
                )
            else:
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        features_values, features_masks = inputs["input_features"], inputs["feature_attention_mask"]
        completion_ids, completion_mask = inputs["completion_ids"], inputs["completion_mask"]
        input_ids = torch.cat([prompt_ids, completion_ids], dim=1)
        attention_mask = torch.cat([prompt_mask, completion_mask], dim=1)
        logits_to_keep = completion_ids.size(1)  # we only need to compute the logits for the completion tokens

        per_token_logps = self._get_per_token_logps(model, input_ids, attention_mask, features_values, features_masks, logits_to_keep)
